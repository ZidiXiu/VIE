...
#     if z_dim == 0:
# #         # non-linear setup
# #         x_pool_new = nonlinear_xbeta(x_pool)
# #         np.random.seed(123)
# #         beta_linear = np.random.uniform(low=0.01, high=0.5, size = len(covariates))
# # #         beta_linear = np.random.normal(size = len(covariates))

# #         beta_linear[ref_cov] = 0.0
# #         xbeta = np.dot(x_pool_new, beta_linear)

#         torch.manual_seed(123)
#         model = Net3(input_size=input_size, h_dim=[4], loglogLink=True, positive=True)
#         model_name = 'simulation_semi'
#         model_path = result_path+"/"+model_name+".pt"
#         print(model)
...
#         batched_x = x_pool.copy()
#         batched_x[:,continuous_variables] = (batched_x.copy()[:,continuous_variables] - norm_mean)/norm_std

#         xbeta=(model(torch.Tensor(batched_x))).squeeze().detach().numpy()

#         df = simulation_cox_weibull(xbeta, x_pool, lambda_=1e-3, nu_=6,cut_bound=cut_bound, seed=seed)
        
#         train = formatted_data_simu_noz(df['x'], df['t'], df['e'], train_idx)
#         test = formatted_data_simu_noz(df['x'], df['t'], df['e'], test_idx)
#         valid = formatted_data_simu_noz(df['x'], df['t'], df['e'], valid_idx)
