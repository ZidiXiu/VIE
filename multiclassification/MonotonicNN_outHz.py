...
# define multiple monotone functions
# suppose z dimension is p, each dimension has k monotone function
class MyMonotoneNN_func(nn.Module):
    def __init__(self, hidden_layers, bias=True):
        super().__init__()
...
# define a general network for all dimensions of z
class MyMonotoneNN_3f(nn.Module):
    def __init__(self, hidden_layers, n_category, bias=True):
        super().__init__()

        self.func1 = MyMonotoneNN_func(hidden_layers, bias)
        self.func2 = MyMonotoneNN_func(hidden_layers, bias)
        self.func3 = MyMonotoneNN_func(hidden_layers, bias)
        # define a linear layer
        # number of dimensions of z * number of functions per dimension
        self.linearity = torch.nn.Linear(4*3, n_category)
        
    def forward(self, z, N, lower_bound, output_Hz = False):
        # latent variable z with dimentions dim_z
        Hz1 = self.func1(z, N, lower_bound)
        Hz2 = self.func2(z, N, lower_bound)
        Hz3 = self.func3(z, N, lower_bound)

        Hz = torch.cat([Hz1, Hz2, Hz3], dim = 1)
...
class MyMonotoneNN_2f(nn.Module):
    def __init__(self, hidden_layers, n_category, bias=True):
        super().__init__()
        ...
        # define a linear layer
        # number of dimensions of z * number of functions per dimension
        self.linearity = torch.nn.Linear(4*2, n_category)
