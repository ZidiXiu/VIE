...
def log_score_marginal(nu, z, mu, logvar, xi_, sigma_, p_ = 0.95, eps=1e-3, nu_lambda=1.0,device='cpu',train_nu=False, opt_nu=None, clipping_value=1e-6):
    ...
    # sample prior z
    n_samples = z.size()[0]
    prior_z = sample_mixedGPD(n_samples,  mu, logvar, xi_, sigma_, p_ = p_, eps=eps, device=device)
...
def testing_VIEVT(dataset, IAF_flow, flow_path, decoder, decoder_path, nu, nu_path, model_name, result_path, eps_dim, L=100, N=100, u_bound=0.99, lower_bound=-5.0,transform = None, norm_mean=0.0, norm_std=1.0, continuous_variables=[],roc_curv = False, pr_curv = False, saveResults=True, device='cpu'):
    ...
    with torch.no_grad():
        ...
        recon_loss, class_acc_ = cross_entropy(pred_risk_batch.float(), batched_e.squeeze().detach().long(), sample_weight=None, class_acc=True)
    ...
    print('====> Test Class Accuracy: {}, F1 score: {:.4f}'.format(class_acc_, test_F1))
