...
from data.simulation_DLN import generate_data, generate_data_semi, formatted_data_simu, saveDataCSV, loadDataCSV

from pathlib import Path

result_path_root = './results/'+"simulationDL"+'/'
...
df, continuous_variables = generate_data(file_path, result_path_root, cut_bound, seed)
print(df['e'].mean())
...
valid_idx = perm_idx[int(n_samples/2):int(4*n_samples/6)]
test_idx = perm_idx[int(4*n_samples/6):n_samples]

train = formatted_data_simu(df['x'], df['t'], df['e'], df['z'],train_idx)
test = formatted_data_simu(df['x'], df['t'], df['e'], df['z'], test_idx)
valid = formatted_data_simu(df['x'], df['t'], df['e'], df['z'], valid_idx)
